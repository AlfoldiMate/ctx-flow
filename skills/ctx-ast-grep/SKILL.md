---
name: ctx-ast-grep
description: The full ast-grep rule-writing workflow — translate a structural question (callers, definitions, code shapes, "async functions without error handling") into a tested rule. Load in the main thread when a query needs more than a one-line pattern; agents carry the short ctx-ast-grep-card instead.
---
